...
description: >
  ...
  organized for code review, refactoring, and writing new Rust. Use when
  reviewing Rust code for ownership, borrow, allocation, panic, or error-handling
  problems; when refactoring or optimizing Rust; when idiomatic API, naming or
  documentation feedback is requested; or when auditing unsafe, concurrency,
  numeric-overflow, or macro code. Covers ownership, error handling, async,
  unsafe, API design, memory, performance, serde, testing, and anti-patterns.
whenToUse: >
  Writing, reviewing, refactoring, or optimizing Rust in this repository, or
  auditing ownership, panic, unsafe, concurrency, or error-handling patterns.
license: MIT
metadata:
  author: leonardomso
  version: "1.5.1"
...
These rules are generic defaults. This repository's own contracts win wherever they differ. Verify against `CLAUDE.md`, `Cargo.toml`, `clippy.toml`, and the current source before acting.

- **Hand-written examples are not this codebase.** Rule examples use `anyhow`, `tracing`, `tokio`, crates and nightly features this project does not depend on. Introducing a new dependency or feature to satisfy a rule is out of scope unless the user asks for it.
- **Panics:** `unwrap_used`, `expect_used`, `dbg_macro`, `todo`, `unimplemented` are `deny` at workspace level, but `clippy.toml` sets `allow-unwrap-in-tests = true` and `allow-expect-in-tests = true`. So `unwrap`/`expect` are denied in production code and fine inside `#[cfg(test)]`; no existing exception may be removed.
- **Error handling:** this crate uses `thiserror` with a hand-written `Error` type. Do not migrate to `anyhow` or `Box<dyn Error>`.
- **Logging:** use the `log` crate through `src/logging.rs`; the Android target routes it to `android_logger`. Do not introduce `tracing`.
- **Async:** this is a synchronous, startup-time binary with no async runtime. `async-*` rules only apply to async code someone actually adds.
- **Targets:** cross-compiled for `aarch64-linux-android`, `armv7-linux-androideabi`, and `x86_64-linux-android`. Never apply `target-cpu=native`, PGO, or other host-specific tuning; use the Android guidance in the `hm-rust-android` skill.
- **Release profile:** do not add `[profile.release]` settings such as `panic = "abort"` — xtask and the rollback transaction depend on the current behavior.
- **Repository rules outrank every rule here,** including the banned symbols in `CLAUDE.md` (`kasumi`, the removed normalization helpers) and the module source-directory read-only invariant.
- **Read selectively.** Open only the rule files relevant to the code under review rather than the whole `rules/` directory.

**REQUIRED BACKGROUND:** Use `hm-mount-safety-review` for mount, unmount, staging, and rollback changes; it takes precedence. Use `hm-verify` for the authoritative local gate list.

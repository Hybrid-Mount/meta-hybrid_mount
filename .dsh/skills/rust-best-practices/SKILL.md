...
## Hybrid Mount / DSH adaptation

Repository Cargo.toml and CLAUDE.md take precedence over generic examples below. This workspace denies unwrap/expect including tests unless an existing explicit lint exception applies. Keep the existing thiserror-based error types; do not introduce anyhow solely because this is a binary. Use actual DSH filesystem and shell tools, not Claude-specific tool names. Read only reference chapters relevant to the task.

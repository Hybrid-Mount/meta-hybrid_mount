...
describe("config store writes", () => {
  ...
  it("tracks all active saves and keeps the saving flag until they settle", async () => {
    const first = deferred<ConfigPatchResult>();
    const second = deferred<ConfigPatchResult>();
    vi.spyOn(API, "saveConfig")
      .mockImplementationOnce(() => first.promise)
      .mockImplementationOnce(() => second.promise);

    configStore.config = { ...DEFAULT_CONFIG, mountsource: "initial" };
    const firstSave = configStore.saveConfig(
      { ...configStore.config, mountsource: "first" },
      { showSuccess: false, showError: false },
    );
    const secondSave = configStore.saveConfig(
      { ...configStore.config, mountsource: "second" },
      { showSuccess: false, showError: false },
    );

    second.resolve(
      patchResult({ ...DEFAULT_CONFIG, mountsource: "second" }),
    );
    await secondSave;

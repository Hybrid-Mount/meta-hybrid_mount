...
Hybrid Mount là siêu mô-đun gắn kết kết hợp dành cho KernelSU và APatch. Trong quá trình khởi động, mô-đun quét các mô-đun khác rồi chọn OverlayFS, Magic Mount, VFS hoặc bỏ qua cho từng mục dựa trên quy tắc toàn cục, quy tắc mô-đun và quy tắc đường dẫn. Thư mục nguồn của mô-đun luôn được xem là dữ liệu đầu vào chỉ đọc.

## Tính năng

- Có thể kết hợp OverlayFS, Magic Mount và VFS theo từng mô-đun hoặc từng đường dẫn.
...
Đường dẫn trong quy tắc được viết tương đối với thư mục gốc của mô-đun. Quy tắc cấp mô-đun và cấp đường dẫn cũng có thể dùng `ignore`; backend mặc định toàn cục chấp nhận `overlay`, `magic` hoặc `vfs`. VFS là đường dẫn tiêm, không phải gắn kết thực. Xung đột về tệp, kiểu hoặc `.replace` sẽ khiến giai đoạn lập kế hoạch khi khởi động báo lỗi ngay lập tức. Thay đổi cấu hình có hiệu lực sau khi khởi động lại.

...
- Kurulumdaki ses tuşu seçimi, modül köklerine tam olarak küçük harfli `overlay` veya `magic` işaretçisi yazar.
...
| Çalışma zamanı yapılandırma uygulaması | Evet | Hayır |
...
- **Çalışma zamanı güncellemeleri** — doğrulanmış yapılandırma değişikliklerini kaydedin ve hemen uygulayın.
...
Özel bind mount'lar modül OverlayFS/Magic Mount işlemlerinden sonra uygulanır. Kaynak ve hedef mevcut olmalı; ikisi de dosya veya ikisi de dizin olmalıdır. İç içe hedeflerde üst dizin hedefi alt hedeflerden önce yazılmalıdır.
...
- `ext4` (varsayılan) — kalıcı, xattr destekli bir ext4 disk imajı oluşturur.
...
WebUI şu dilleri içerir: İngilizce, İspanyolca, İtalyanca, Japonca, Rusça, Ukraynaca, Vietnamca, Basitleştirilmiş/Geleneksel Çince ve Türkçe (`tr-TR`).
...
| `custom_mounts` | array | `[]` | Modüller uygulandıktan sonra çalıştırılan açık bind mount listesi. |
...
İşaretçi adları tam, küçük harfli ve büyük/küçük harfe duyarlıdır.
...
```text
hybrid-mount [--json] [--config <path>] [--no-daemon] <command>
```
...
| Komut | Açıklama |
| --- | --- |
| `mount` | Yapılandırmayı yükler ve bağlama hattını çalıştırır. |
| `daemon launch` | Daemon'u başlatır. |
| `daemon status` | Daemon sağlığını gösterir. |
| `api modules-list` | Modülleri JSON olarak listeler. |
| `api system-info` | Sistem ve çalışma zamanı durumunu gösterir. |
| `api config-get` | Etkin yapılandırmayı döndürür. |
| `api save-full-config --json <JSON>` | Tam yapılandırmayı doğrular ve kaydeder. |
| `api config-reset` | Yapılandırmayı varsayılana sıfırlar. |
| `emulated-soft-reboot` | Eski bağlamaları ayırır ve soft reboot akışını çalıştırır. |

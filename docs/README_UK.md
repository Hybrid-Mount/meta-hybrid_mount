...
Скрипт копіює джерела до `fs/hybridmount/` і додає їх до `fs/Makefile` та `fs/Kconfig`; увімкніть `CONFIG_HYBRIDMOUNT=y`, щоб вбудувати його, або `=m`, щоб зібрати як модуль. `--cleanup` скасовує всі зміни. Дерево, у яке вже інтегровано NoMount, відхиляється: обидві реалізації перехоплюють операції з inode, і ядро не завадить їм співіснувати, оскільки вони реєструють різні типи ключів.

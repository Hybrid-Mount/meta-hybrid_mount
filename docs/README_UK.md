...
Пакети публікуються у двох варіантах. Якщо не зазначено інше, цей README описує варіант `lite`.
...
| Варіант | Бінарний файл | WebUI | Daemon / CLI | Сценарій |
|---------|---------------|-------|--------------|----------|
| **Lite** | Так | Так | Так | Для користувачів, яким потрібні WebUI та повний рушій політик без LKM-based stealth. |
| **Nano** | Так | Ні | Ні | Для конфігураційного монтування без runtime-daemon, WebUI та CLI. |

### Lite

Варіант `lite` є варіантом за замовчуванням. Він зберігає WebUI, daemon, CLI, OverlayFS і Magic Mount. Він підходить, якщо ядро не підтримує зовнішні LKM або runtime-можливості hide/spoof не потрібні.
...
| CLI | Так | Ні |
| Daemon | Так | Ні |
| Кеш конфігурації та runtime-apply | Так | Ні |
| Cargo features | тільки `control-plane` | немає |
| Розмір ZIP (прибл.) | ~2 MB | ~1 MB |

## Можливості

- **Два backend-и, один рушій політик**: призначення OverlayFS або Magic Mount на рівні окремих шляхів.
...
| `default_mode` | `overlay` \| `magic` | `overlay` | Глобальна політика за замовчуванням. |
...
Hybrid Mount читає `config.toml`, знаходить inventory модулів, будує план монтування за правилами шляху, модуля та глобальними правилами, після чого виконує його через OverlayFS або Magic Mount. Виконавець керується **типізованим скінченним автоматом** (`src/core/controller.rs`): `MountController<Init> → StorageReady → Planned → Executed`. Кожен перехід представляє один етап конвеєра. Варіант Lite зберігає runtime-стан і відкриває до нього доступ через WebUI та CLI.
...
- `src/mount`: backend-и OverlayFS і Magic Mount.
...
Кожна зміна повинна проходити: `cargo fmt --all -- --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test --all-targets --workspace`, WebUI `pnpm lint` + `pnpm test`, і перевірку заголовка ліцензії. Також переконайтеся, що комбінації **lite** (`--no-default-features --features control-plane`) та **nano** (`--no-default-features`) компілюються. Код daemon/CLI/WebUI має бути за `#[cfg(feature = "control-plane")]`.
...
Ліцензовано за [Apache-2.0](https://github.com/Hybrid-Mount/meta-hybrid_mount/blob/main/LICENSE).

...
1. **This only builds for `aarch64-linux-android`.** There is no host-platform build. Building/running on Windows or x86 Linux is **not** a supported workflow.
   ...
   - Building requires: **Rust nightly** (pinned by `rust-toolchain.toml`), **Android NDK r27+**, and **`cargo-ndk`**. WebUI needs **Node 20.19+ or 22.12+** and **pnpm 10.34.5**.
...
- **Late-load is supported**: `KSU_LATE_LOAD=1` (KernelSU jailbreak / locked-BL mode) no longer aborts install or startup. KernelSU runs `module/emulated-soft-reboot.sh` during its emulated soft reboot; the script calls `hybrid-mount emulated-soft-reboot`, which detaches the previous run's mounts before `metamount.sh` mounts again. The same detach also runs at startup when `KSU_LATE_LOAD=1`, so re-mounts never stack. Cleanup ownership must remain exact: overlay options must reference this project's workspace/data root, workspaces must match `/mnt/hm_<10 chars>` or `/debug_ramdisk/hm_<10 chars>` exactly, and Magic/custom targets must come from persisted runtime state or the current custom-bind configuration. Never treat the shared KernelSU/APatch mount source, a module-directory prefix, or a managed-partition guess as proof of ownership.

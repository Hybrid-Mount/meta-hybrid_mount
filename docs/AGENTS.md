...
- `core/` — `inventory/` (module discovery), `ops/` (plan + per-backend executors), `daemon/` (Unix socket for CLI + TCP/SSE HTTP for WebUI), `api/` (WebUI payload builders), `startup/` (boot orchestration), `storage/` (ext4 image / tmpfs), `runtime_state.rs`, `kasumi_coordinator.rs`.

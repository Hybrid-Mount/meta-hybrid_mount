...
以下事项从开发收尾记录保留；记录反映截至 2026-09-12 的本地交接信息，不代表重新核验了远端 Issue 状态或完成了新一轮审查。

- Issue #409：旧配置 `daemon_startup_mode` 的兼容修复已提交（`1ba20add`），仍需确认报告者设备升级后重启正常、挂载设置和模块/路径规则保留。报告与后续反馈见 [Issue #409](https://github.com/Hybrid-Mount/meta-hybrid_mount/issues/409)。
- Android 实际挂载、LKM 加载和重启恢复仍需真机验证；主机测试与三架构编译不能替代设备验证。
- 后端遗留问题继续按下表跟踪，尤其 HM-RUST-009 尚未闭环；修复前应针对当前代码重新验证。

...
|------|--------|-------|----------------|----------|
| **Lite** | 是 | 是 | 是 | 需要 WebUI 與完整策略引擎，但不需要 LKM 級 stealth 功能的使用者。 |
| **Nano** | 是 | 否 | 否 | 只想透過設定檔控制掛載、不需要常駐守護行程或 WebUI 的使用者。 |

### Lite

`lite` 版本（`--no-default-features --features control-plane`）是預設版本，保留 WebUI、守護行程、CLI、OverlayFS 與 Magic Mount。適合需要圖形介面與完整策略引擎的環境。
...
| `default_mode` | `overlay` \| `magic` | `overlay` | 全域預設掛載策略。 |
...
Hybrid Mount 的執行流程是：讀取 `config.toml`，掃描模組清單，依照路徑、模組與全域策略產生掛載計畫，再交由 OverlayFS 或 Magic Mount 執行器處理。執行器由 **typestate 狀態機**（`src/core/controller.rs`）驅動：`MountController<Init> → StorageReady → Planned → Executed`，每次狀態轉換代表一個管線階段。Lite 版本會將執行階段狀態持久化，並透過守護行程提供 WebUI 與 CLI 存取。
...
- `src/mount`：OverlayFS 與 Magic Mount 後端。
- `src/sys`：低階 mount syscall。
...
每次變更必須通過以下 CI 檢查：`cargo fmt --all -- --check`、`cargo clippy --all-targets -- -D warnings`、`cargo test --all-targets --workspace`、WebUI `pnpm lint` + `pnpm test`，以及授權標頭檢查。也請確保 **lite**（`--no-default-features --features control-plane`）與 **nano**（`--no-default-features`）版本能夠編譯。涉及 daemon/CLI/WebUI 的程式碼須置於 `#[cfg(feature = "control-plane")]` 之後。

...
- **OverlayFS** — 使用分层挂载合并模块内容。
- **Magic Mount** — 使用 bind mount 直接替换目标路径。

内置 **SolidJS WebUI**，支持图形化管理、实时状态监控和配置编辑。

发布包分为两个版本 — 详见 [构建版本](#构建版本) 对比。除非另有说明，下面内容默认描述 Lite 版本。
...
Hybrid Mount 发布为两个版本（flavor），分别面向不同使用场景：

| 版本 | 二进制 | WebUI | 守护进程 / CLI | 适用场景 |
|------|--------|-------|---------------|----------|
| **Lite（默认）** | 是 | 是 | 是 | 默认发行版：WebUI、守护进程、CLI，以及 OverlayFS 和 Magic Mount 两个后端。 |
| **Nano** | 是 | 否 | 否 | 极简主义用户，仅需通过配置文件控制挂载，无需运行时守护进程。 |

### Lite

Lite 是默认发行版，包含 SolidJS WebUI、Unix socket 守护进程（HTTP/SSE）、CLI，以及 OverlayFS 和 Magic Mount 两个后端：

- 需要 WebUI 和完整策略引擎。
- 想要更小的下载体积，同时保留 WebUI 和守护进程管理界面。

Lite 构建仅使用 `control-plane` feature（`--no-default-features --features control-plane`）。
...
- **无 WebUI** — 包中移除了 `webroot/` 和 `launcher.png` 资源。
- **纯挂载操作** — 二进制在启动时运行，按照配置完成所有挂载后终止。
- **默认模式为 `magic`** — Nano 的预置配置中 `default_mode = "magic"`，在没有守护进程管理 ext4 镜像时优先使用 bind mount。
- **模块模式标记** — 安装时通过音量键选择后，会在每个受管理模块根目录写入空的 `overlay` 或 `magic` 标记文件，Nano 运行时直接读取它们，而不再依赖白名单。标记文件名必须使用完全一致的小写拼写。
...
- **运行时配置更新** — 配置补丁通过严格校验后可持久化并立即生效。
- **显式失败报告** — 无效状态和配置会立即报错；配置重置必须显式调用 `api config-reset`。
...
4. 首次安装时直接选择默认挂载模式：音量加选择 OverlayFS，音量减选择 Magic Mount，10 秒无操作则选择 OverlayFS。这是安装器唯一的交互步骤；Nano 会跳过此步骤。
5. 重启设备。Hybrid Mount 将自动检测运行环境并应用所选的默认策略。
...
Hybrid Mount 还会识别模块目录中的标记文件。这些标记应为普通文件；运行时只使用文件名判断。标记文件名严格区分大小写，必须使用下表列出的准确名称。
